...
public class DummyControllerTest {
	...
	public String deleteUser(@PathVariable int id) {
		...
		userRepository.deleteById(id);
			...
			return "삭제를 실패했습니다. 해당 ID는 DB에 없습니다.";
		}
...
//		userRepository.save(user);
	...
	public User detail(@PathVariable int id) {
		...
		// user/4를 찾을 때 내가 DB에서 못찾아오게되면 user가 null이 됨
		// 그럼 return null; 이 될 것임. 그럼 문제가 생길 것임
		// Optional로 너의 User 객체를 감싸서 가져올테니 null인지 아닌지 판단해서 return 할 것!!
		...
		// 요청 : 웹브라우저, user객체 : 자바오브젝트
		// json으로 변환(웹브라우저가 이해할 수 있는 데이터)
		// 스프링부트 = MessageConverter라는 애가 응답시에 자동 작동
		// 만약에 자바 오브젝트를 리턴 하게되면 MessageConverter가 Jackson 라이브러리 호출해서
		// user 객체를 json으로 변환해서 브라우저에 던짐
		return user;
	}
